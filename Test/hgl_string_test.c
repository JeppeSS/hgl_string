...
bool
hgl_string_concat_valid_strings_concated( void )
{
  char *p_hgl_string = hgl_string_concat( "foo", "bar" );
  if( hgl_string_length( p_hgl_string ) == 6 )
  {
    hgl_string_destroy( p_hgl_string );
    return true;
  }
...
bool
hgl_string_concat_one_invalid_string( void )
{
  char *p_hgl_string = hgl_string_concat( NULL , "bar" );
  if( !p_hgl_string )
  {
    return true;
  }
...
main( void )
{
  hgl_unit test_cases[] = {
    ...
    { "hgl_string_concat_valid_strings_concated",
       hgl_string_concat_valid_strings_concated },
    
    { "hgl_string_concat_one_invalid_string",
       hgl_string_concat_one_invalid_string }
  };

  hgl_unit_run( test_cases, 6 );
